...
export function AuthProvider({ children }) {
  ...
  useEffect(() => {
    ...
    const handleActivity = (event) => {
      event.preventDefault(); // Prevent default to avoid interference
      handleUserActivity();
    };

    // Add event listeners
    activityEvents.forEach(event => {
      document.addEventListener(event, handleActivity, { passive: false, capture: true });
    });

    // Cleanup function
    return () => {
      activityEvents.forEach(event => {
        document.removeEventListener(event, handleActivity, { passive: false, capture: true });
      });
      resetLogoutTimer();
    };
